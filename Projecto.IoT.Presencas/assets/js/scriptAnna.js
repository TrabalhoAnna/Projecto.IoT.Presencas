...
//apresentação da disciplina seleciona no Aluno
var CursoSelecAnterior;//Para guardar qual a disciplina guardade anteriormente
function Cursoselect(Select) {

    var Curso = document.querySelectorAll('[id=Curso]');//Vai buscar todos os elementos com o id=Disciplina
    var CursoS = document.getElementsByClassName('DispSelect');//Vai buscar todos os elementos com o class=DispSelect
    var CursoTab = document.getElementById(Select.textContent);//Pega o elemento com o id = ao conteudo do elemento clicado
    var CursoTabAnterior = document.getElementById(CursoSelecAnterior);//Pega o elemento com o id = ao conteudo do elemento clicado anteriormente

    if (CursoS.length === 0) {//Caso nao exista um elemento com o id=DispSelect altera a classe do elemente clicado para DispSelect e mostra um novo card
        Select.className = "DispSelect";
        CursoTab.style.display = "block";
    }
    else {//Caso já exista
        CursoS[0].className = "Disp";//Altera a classe do elemento que contenha como classe=DispSelect
        CursoTabAnterior.style.display = "none";//Escondendo o card

        for (var i = 0; i < Curso.length; i++) {//Percorre todos os elementos que tenham como id=Disciplina
            if (Curso[i] === Select) {//Caso o elemento Selecionado for o mesmo que o elemento a ser percorrido
                Curso[i].className = "DispSelect";//Altera a Classe desse elemento para DispSelect
                CursoTab.style.display = "block";//Mostra o card
            }
            else {//Caso o elemento Selecionado não seja o mesmo que o elemento a ser percorrido
                Curso[i].className = "Disp";//Mantem a classe Disp
            }
    ...
    CursoSelecAnterior = Select.textContent;//Salva o conteudo do elemento selecionado
}
